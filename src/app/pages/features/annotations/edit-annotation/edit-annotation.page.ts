...
export class EditAnnotationPage {
  ...
  ionViewWillEnter() {
    this.id = this.annotationsService.annotationIdSelected;
    if (this.id != null && this.route.url.indexOf('/new') < 0) {
    ...
    this.annotationsService.saveAnnotation().subscribe(
      (response) => {
        ...
        if (redirect) {
          ...
          this.annotationsService.navigateToAnnotation(response.id);
        }
